...
class Order {
    ...
    async create(req, res, next, type) {
        try {
            ...
            if (type === 'admin') {
                ...
                //если заказ сделал пользователь, состав заказа получается из корзины
                //а id из req.auth.id(если есть)
                if (!req.signedCookies.basketId) throw new Error('Ваша корзина пуста и все тут')
    ...
    async userGetAll(req, res, next) {
        try {
            const order = await OrderModel.getAll(req.auth.id)
            res.json(order)
        } catch(e) {
            next(AppError.badRequest(e.message))
        }
